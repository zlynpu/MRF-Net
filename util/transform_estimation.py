...
def update_pcd(pts, trans):
  R = trans[:3, :3].to(pts.device)
  T = trans[:3, 3].to(pts.device)
...
def build_linear_system(pts0, pts1, weight):
  npts0 = pts0.shape[0]
  A0 = torch.zeros((npts0, 6)).to(pts0.device)
  A1 = torch.zeros((npts0, 6)).to(pts0.device)
  A2 = torch.zeros((npts0, 6)).to(pts0.device)
  A0[:, 1] = pts0[:, 2]
  A0[:, 2] = -pts0[:, 1]
  A0[:, 3] = 1
  A1[:, 0] = -pts0[:, 2]
  ...
  A2[:, 1] = -pts0[:, 0]
  A2[:, 5] = 1
  ww1 = weight.repeat(3, 6).to(pts0.device)
  ww2 = weight.repeat(3, 1).to(pts0.device)
